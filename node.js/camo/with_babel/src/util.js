...
export function create(movie) {
  Movie.create(movie).save().then(savedRes => {
...
export function readAllByTitle(title) {
  Movie.find({title: title}).then(foundMovies => {
...
export function updateByTitle(title, rating) {
  Movie.findOneAndUpdate({title: title}, {rating: rating}).then(foundMovie => {
    if(foundMovie) {
...
export function deleteByTitle(title) {
  Movie.deleteOne({title:title}).then(count => {
